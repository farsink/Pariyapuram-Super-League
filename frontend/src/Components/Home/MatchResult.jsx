...
const MatchResult = () => {
  ...
  const HomeScorer =
    match?.goalScorers?.filter((scorer) => scorer.team === match?.homeTeam?._id) || [];


  const AwayScorer =
    match?.goalScorers?.filter((scorer) => scorer.team === match?.awayTeam?._id) || [];
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "line-ups":
        return <MatchLineup />;
      default:
        return <div className="p-4 text-center text-gray-500">Content for {activeTab} tab</div>;
    }
  };
  return (
      ...
      <div className="bg-[#f9f9f9] text-[#37003C] p-4 shadow-sm">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl md:text-3xl">
            <span className="font-normal">Matchday</span> <span className="font-bold">Live</span>
          </h1>
          <div className="flex flex-col items-end">
            <span className="text-sm font-medium">{match?.round.toUpperCase()}</span>
          ...
          <div className="p-4 border-b bg-Purple rounded-md ">
            ...
            <div className="hidden md:flex flex-wrap items-center gap-4 text-sm text-gray-200 mt-2">
              <div>
                ...
                <span> • </span> Kick Off: {new Date(match?.date).toLocaleTimeString()}
              </div>
              <div>Att: 17,154</div>
              ...
              <div className="flex flex-col items-center mx-4">
                {match?.status === "completed" ? (
                  ...
                  <>
                    ...
                    <div className="text-sm font-medium">
                      <span className="uppercase">{new Date(match?.date).toLocaleTimeString()}</span>
                    </div>
                  </>
                )}
              </div>
            ...
            <div className="mt-6 pt-6 border-t-2 border-gray-200">
              <div className="max-w-4xl mx-auto flex justify-between px-4">
                ...
                <div className="text-center md:text-left">
                  <h3 className="text-xs uppercase text-gray-500 mb-2">{match?.homeTeam?.name}</h3>
                  <div className="text-sm">
                    {HomeScorer?.reduce((acc, scorer) => {
                      const existing = acc.find((s) => s.player.name === scorer.player.name);
                      if (existing) {
                        existing.count += 1;
                      } else {
                      ...
                      <p key={scorer.player.name}>
                        {scorer.player.name?.replace(/(^\w{1})|(\s+\w{1})/g, (letter) =>
                          letter.toUpperCase()
                        )}
                ...
                <div className="text-center md:text-right">
                  <h3 className="text-xs uppercase text-gray-500 mb-2">{match?.awayTeam?.name}</h3>
                  <div className="text-sm">
                ...
                {["live", "news", "line-ups", "stats"].map((tab) => (
                  <button
                    ...
                    className={`
                      ...
                      ${activeTab === tab ? "text-[#37003C]" : "text-gray-500 hover:text-gray-700"}
