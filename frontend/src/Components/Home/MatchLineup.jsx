...
const MatchLineup = ({ match }) => {
  ...
  const awayteamPlayers = match.awayTeam.players.map((player, index) => ({
  ...
  const awayTeamSubs = match.awayTeam.players.slice(5).map((player) => ({
    number: Math.floor(Math.random() * 19) + 1,
    name: player.name,
    position:
      player.position ||
      ["RB", "CM", "LW", "ST"][Math.floor(Math.random() * 4)],
  }));
    ...
    <div className='p-4 max-w-7xl mx-auto bg-white'>
      ...
      <div className='grid grid-cols-2 gap-3 sm:gap-8'>
        ...
        <div className='w-full'>
          ...
          />
          <SubsList subs={awayTeamSubs} title='Substitutes' />
        </div>
        ...
        <div className='w-full'>
          ...
          />
          <SubsList subs={HomeTeamSubs} title='Substitutes' />
        </div>
      </div>
    </div>
  );
